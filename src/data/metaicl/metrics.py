...
def compute_metrics(
    ...
    rouge_results = evaluate.load("rouge").compute(predictions=decoded_preds,
    ...
    input_lens = [
        np.count_nonzero(inputs != tokenizer.pad_token_id) for inputs in eval_preds.inputs
    ]
    results["input_len"] = np.mean(input_lens)
...
def get_compute_metrics_fn(comp_token: int, tokenizer: PreTrainedTokenizer,
                           args: Arguments) -> Callable:
    return functools.partial(compute_metrics, comp_token=comp_token, tokenizer=tokenizer, args=args)
