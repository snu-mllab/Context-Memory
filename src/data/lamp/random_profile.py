...
class LaMPRandomProfileConfig(datasets.BuilderConfig):
    def __init__(
        self,
        max_tok_len=100,
        cutoff_ratio=0.9,
        ...
        **kwargs,
        ...
        time_based = "time" in dataset_name
...
class LaMPRandomProfile(datasets.GeneratorBasedBuilder):
    """LaMP Dataset."""

    VERSION = datasets.Version("1.0.0")
    BUILDER_CONFIG_CLASS = LaMPRandomProfileConfig
    ...
    def _generate_examples(
        ...
        profile_permutation = np.random.permutation(num_data)
        ...
        if outputs is not None:
            for i, (question, output) in enumerate(zip(questions, outputs)):
                ...
                profile_idx = profile_permutation[i]
                profile = questions[profile_idx]["profile"]

                if "date" in profile and isinstance(profile["date"], int):
                    profile["date"] = str(profile["date"])

                if ratio[profile_idx] < cutoff_ratio:
                    continue

                profile = [p for j, p in enumerate(profile) if cutoff[profile_idx][j]]
