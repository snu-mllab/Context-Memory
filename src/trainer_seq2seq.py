...
class CompSeq2SeqTrainer(Seq2SeqTrainer):
    def prediction_step(
        ...
        if not is_encoder_decoder and "prompt_input_ids" in inputs:
            ...
            original_inputs = {
                k: inputs[k]
                for k in ["input_ids", "attention_mask", "attention_mask_comp"]
            }
    ...
    def _loglikelihood_seq2seq(self, batch, check=False, split_batch=-1):
        ...
        if split_batch > 0:
            ...
            for i in range(0, len_batch, batch_split_size):
                ...
                logits_i = logits_i.to(dtype=torch.float, device=torch.device("cpu"))
            ...
            logits = logits.to(dtype=torch.float, device=torch.device("cpu"))
            log_probs = F.log_softmax(logits, dim=-1)
